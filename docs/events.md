...
Ядро обрабатывает события в `EventsConsumer.handle`: проверяет `notification.requested` и отдаёт адресованные события в SSE. Обработчик модуля, например начисление очков по `scenario.completed`, подключается туда же вместе с первым таким обработчиком. Это инфраструктура ядра, поэтому правку согласуйте с техлидом.

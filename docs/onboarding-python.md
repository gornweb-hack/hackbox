...
- **ИИ-помощник работает в зоне Python** ([AGENTS.md](../AGENTS.md), раздел «Зоны ответственности агента»). Он правит только Python-модули, а `core/`, `web/` и Go-модули только читает. Если нужна правка в ядре или на фронте, агент опишет, что поменять, а вы передадите это владельцу.
